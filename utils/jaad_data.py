'''
imitate to rewrite jaad_data.py file.
'''
import sys
import pickle
import cv2
...
class JAAD(object):
    def __init__(self, data_path='/home/robert/桌面/Pedestrian_Crossing_Intention_Prediction-main', regen_pkl=False):
        '''param:
        data_path: Path to the folder of the dataset, default is current dir.
        regen_pkl: Whether to regenerate the database.
        '''
        self._name = 'JAAD'
        ...
        assert exists(self._jaad_path), 'Jaad path does not exist: {}'.format(self._jaad_path)
    ...
    def _get_default_path(self):
        '''
        return default data_path where jaad_raw files are expected to be palced.
        '''
        return 'dataset/jaad'
    ...
    def _get_video_ids(self):
        ...
        :return: the list of all video ids
        '''
    ...
    def extract_and_save_images(self):
        ...
        #eg: clip_path: JAAD_clips/---->video_0001.mp4
        #                           |-->video_0002.mp4
        #                           |-->video_0003.mp4
        #                           |-->video_0004.mp4
        #                           --->video_0005.mp4
        #get videos = [video_0001, video_0002, video_0003, video_0004, video_0005]
        for vid in videos:
            path_to_file = join(self._annotation_path, vid + '.xml')
            #path: annotations/vidXXX.xml
            print(vid)
            tree = ET.parse(path_to_file)#load element tree.
            num_frames = int(tree.find('./meta/task/size').text)
            video_clip_path = join(self._clips_path, vid + '.mp4')
            #path: JAAD_clips/vidXXX.mp4
    ...
    def _map_text_to_scalar(self, label_type, value):
        ...
        :param: label type: the label type
    ...
    def _get_annotations(self, vid):
        ...
        for t in ped_tracks:
            ...
            annotations[ped_annt][new_id] = {'old_id': old_id, 'frames': [], 'bbox': [], 'occlusion': []}
            if 'pedestrian' in old_id:
                annotations['ped_annotations'][new_id]['behavior'] = {
                    'cross': [], 'reaction': [], 'hand_gesture': [], 'look': [], 'action': [], 'nod': []
                }
            else:
            ...
            for b in boxes:
                ...
                for beh in annotations['ped_annotations'][new_id]['behavior'].keys():
                    annotations[ped_annt][new_id]['behavior'][beh].append(self._map_text_to_scalar(beh, b.find('./attribute[@name=\"' + beh + '\"]').text))
        return annotations

    def _get_ped_attributes(self, vid):
        '''
        Generates a dictionary of attributes by parsing the video XML file.
        :param vid: the id of the video to parse
    ...
    def _get_ped_appearance(self, vid):
        '''
        Generates a dictionary of appearance annotations by parsing the video XML file.
        :param vid: the id of video to parse. The labels are follows:
            ...
            -backpack - presence of a backpack(wron on the back, not held in hand).
            -bag_hand, bag_elbow, bag_shoulder - whether bag(s) are held in a hand, on a bent elbow or worn on a shoulder.
            -bag_left_side, bag_right_side - whether bag(s) appear in the left/right side of the pedestrian body.
            -cap, hood - headwear.
            -umbrella, phone, baby, object - various things carried by the pedestrians.
            -stroller/cart - objects being pushed by the pedetrian.
            -bicycle/motorcycle - for pedestrians riding ot walking these vehicles.
    ...
    def _get_vehicle_attributes(self, vid):
        '''
        Generate a dictionary of vehicle attributes by parsing the video XML file.
        :param vid: the id of the video to parse.
        :return: a dictionary of vihecle attributes.
        '''
    ...
    def generate_database(self):
        '''
        Generate a database of jaad database by integrating all annotations
        ...
        '''
        print('------------------------------------------------')
        print('Generating databse for jaad')
        #Generates a list of behavioral xml file names for videos
        cache_file = join(self.cache_path, 'jaad_database_old_Jaad.pkl')
    ...
    def get_data_stats(self):
        '''
        Generate statistics for jaad database.
        '''
    ...
    def balance_samples_count(self, seq_data, label_type, random_seed=42):
        '''
        balance the number of positive and negative samples by randomly sampleing
        from the more represented samples. Only works for binary classes.
        :param seq_data: the sequence data to be balanced.
        :param label_type: the label type based in which the balancing takes place.
        ...
        new_seq_data = {}
        #finds the indicies of the samples with larger quantity
        if num_neg_samples == num_pos_samples:
            print('Positive and negative are already balanced.')
            return seq_data
        else:
    ...
    def _get_random_pedestrian_ids(self, image_set, ratios=None, val_data=True, regen_data=False, sample_type='all'):
        '''
        Generates and save a database of activities for all pedestrians.
        :param image_set: The data split to return.
        :param ratios: The ratios to split the data. There should be 2 ratios(or 3 if val_data is true) and they should sum to 1. e.g. [0.4, 0.6], [0.3, 0.5, 0.2].
        :param val_data: Whether to generate validation data.
        :param regen_data: Whether to overwrite the existing data.
        :return: The random smaple split.
        '''
        ...
        if exists(cache_file) and not regen_data:
            ...
            with open(cache_file, 'rb') as fid:
                ...
                if val_data:
                    assert len(rand_samples['ratios']) == 3, 'The existing random samples does not have validation data.\n Please try again by setting regen_data = True'
                if ratios is not None:
                    assert ratios == rand_samples['ratios'], 'Specified ratios {} does not match te ones in existing file {}.\n\
        ...
        if val_data:
            ...
            print('Number of val tracks %d' % len(test_samples))
    ...
    def _get_kfold_pedestrian_ids(self, image_set, num_folds=5, fold=1, sample_type='all'):
        ...
        '''
        assert image_set in ['train', 'test'], 'For Kfold data split, image_set should be either \'train\' or \'test\''
    ...
    def _get_data_ids(self, image_set, params):
        ...
        :param image_set: Image_set to generate data.
    ...
    def _squarify(self, bbox, ratio, img_width):
        ...
        if bbox[0] < 0:
            bbox[0] = 0
        #check whether the new bbox goes beyond image boarders
        #if this is the case, the bbox is shifted back.
        if bbox[2] > img_width:
    ...
    def get_detection_data(self, image_set, method, occlusion_type=None, file_path='data/', **params):
        '''
        Generate data for pedestrian detection algorithms
            ...
            for pid in annotations[vid]['ped_annotaions']:
                ...
                if 'p' in pid:
                    ...
                    if image_set in ['train', 'val']:
                        continue
                imgs = [join(self._jaad_path, 'images', vid, '{:05d}.png'.format(f)) for f in annotations[vid]['ped_annotations'][pid]['frames']]
                ...
                for i, b in enumerate(boxes):
                    ...
                    if height_rng[0] <= bbox_height <= height_rng[1]:
                        if (occlusion_type == None and occlusion[i] == 0) or (occlusion_type == 'part' and occlusion[i] < 2) or (occlusion_type == 'full'):
                            if squarify_ratio:
                                b = self._squarify(b, squarify_ratio, img_width)
                            ped_samples[imgs[i]].append({'width': img_width,
                                                'height': img_height,
                                                'tag': pid,
                                                'box': b,
    ...
    def _print_dict(self, dic):
        '''
        Prints a dictionary, one key_value pair per line.
    ...
    def _height_check(self, height_rng, frame_ids, boxes, images, occlusion):
        '''
        Checks whether the bounding boxes are within a given height limit. If not, it will adjust the length of data sequences accordingly.
    ...
    def generate_data_trajectory_sequence(self, image_set, **opts):
        ...
        :param opts:
                'fstride': Frequency f sampling from the data.
                'sample_type': Whether to use 'all' pedestrian annotations or the ones with 'beh'avior only.
                'subset': The subset of data annotations to use. Options are: 
                                                                        'default': Includes high resolution and high visibility videos.
                                                                        'high_visibility': Only videos with high visibility(include low resoltion videos).
                                                                        'all': Uses all videos.
                'height_rng': The height range of pedestrian to use.
                'squarify_ratio': The width/height ratio of bounding boxes. A value between (0,1]. 0->the originalratio is used.
                'data_split_type': How to split the data. Options:
                                                                'default': predefined sets,
                                                                'random': randomly split the data.
                                                                'kfold': k-fold data split(NOTE: only train/test splits).
                'seq_type': Sequence type to generate. Options:
                                                            'trajectory': generates tracks.
                                                            'crossing': generates tracks uo to 'crossing_point',
                                                            'intention': generates tracks similar to human experiments.
        ...
        assert all(k in params for k in opts.keys()), 'Wrong option(s). Choose one of the following: {}'.format(list(params.keys()))
        params.update(opts)
        print('--------------------------------------------------------')
    ...
    def _get_trajectories(self, image_set, annot_database, **params):
        ...
        :param image_set: Data split.
        :param annot_database: The annotations database.
        ...
        for vid in sorted(video_ids):
            ...
            for pid in sorted(annot_database[vid]['ped_annotations']):
                ...
                images = [join(self._jaad_path, 'images', vid, '{:05d}.png'.format(f)) for f in pid_annots[pid]['frames']]
    ...
    def _get_crossing(self, image_set, annot_database, **params):
        ...
        :param annot_database: Annotations database.
