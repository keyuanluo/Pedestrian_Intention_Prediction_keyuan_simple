...
def train(model, train_loader, class_criterion, reg_criterion, optimizer,
    ...
    训练函数（无验证集）：
      ...
      - 使用训练集上指标作为性能提升标准进行 early stopping
    """
    ...
    for epoch in range(epochs):
        ...
        for bbox, label, vel, traj, acc_data in train_loader:
            # 将 label、bbox、vel、acc_data 分别送入 GPU
        ...
        # 以训练集的分类损失作为 early stopping 指标
        avg_cls_loss = cls_losses / nb_batches_train
        if best_train_loss > avg_cls_loss:
            best_train_loss = avg_cls_loss
            num_steps_wo_improvement = 0
            save_times += 1
...
def test(model, test_data):
    """
    测试函数：
      - DataLoader 返回 5 个元素: (bbox, label, vel, traj, acc_data)
      - 返回模型对测试集的预测和标签
    """
    print('Testing...')
    with torch.no_grad():
        model.eval()
        step = 0
        for bbox, label, vel, traj, acc_data in test_data:
            label = label.reshape(-1, 1).to(device).float()
            bbox = bbox.to(device)
            vel = vel.to(device)
            acc_data = acc_data.to(device)
    ...
    if num_neg_samples == num_pos_samples:
        ...
        for i in range(num_samples):
            if d['activities'][i][0][0] == gt_augment:
                ...
                flipped_bbox = [np.array([img_width - c[2], c[1], img_width - c[0], c[3]]) for c in flipped_bbox]
        ...
        print(f'Balanced: Postive: {num_pos_samples} \t Negative: {len(d["activities"]) - num_pos_samples}\n')
