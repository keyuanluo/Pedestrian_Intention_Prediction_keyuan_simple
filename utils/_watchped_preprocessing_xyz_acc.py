...
def train(model, train_loader, valid_loader, class_criterion, reg_criterion, optimizer, checkpoint_filepath, writer,
    ...
    for epoch in range(epochs):
        ...
        for bbox, label, vel, traj, acc in train_loader:
            ...
            pred, point, s_cls, s_reg = model(bbox, vel, acc)
        ...
        if best_valid_loss > valid_cls_loss:
            ...
            print(str(num_steps_wo_improvement) + '/3000 times Not update.\n')
...
def evaluate(model, val_data, class_criterion, reg_criterion):
    ...
    with torch.no_grad():
        model.eval()
        acc = 0
        for bbox, label, vel, traj, acc in val_data:
            label = label.reshape(-1, 1).to(device).float()
            bbox = bbox.to(device)
            vel = vel.to(device)
            acc = acc.to(device)
...
def test(model, test_data):
    ...
    with torch.no_grad():
        model.eval()
        step = 0
        for bbox, label, vel, traj, acc in test_data:
            label = label.reshape(-1, 1).to(device).float()
            bbox = bbox.to(device)
            vel = vel.to(device)
            acc = acc.to(device)

            pred, _, _, _ = model(bbox, vel, acc)
...
def balance_dataset(dataset, flip=True):
    ...
    if num_neg_samples == num_pos_samples:
        ...
        # 2) 对那些标签为 gt_augment 的样本进行“镜像翻转”并添加到 d 中
        for i in range(num_samples):
            if d['activities'][i][0][0] == gt_augment:
                # (a) 翻转 center
...
def tte_dataset(dataset, time_to_event, overlap, obs_length):
    ...
    if isinstance(time_to_event, int):
        for k in d_obs.keys():
            if k == 'acc':  # 如果加速度序列长度和 bbox 一致，就跟 bbox 一样做截取
                for i in range(len(d_obs[k])):
                    # 观察序列
                    d_obs[k][i] = d_obs[k][i][- obs_length - time_to_event: -time_to_event]
                    # tte 序列
                    d_tte[k][i] = d_tte[k][i][- time_to_event:]
            elif k not in ['bbox','pid','activities','image','vehicle_act','center']:
                # 如果有别的字段，但你不想裁剪，就跳过
                continue
            else:
                for i in range(len(d_obs[k])):
        ...
        # 这里是 time_to_event 为列表 [30,60] 时的处理逻辑
        olap_res = obs_length if overlap == 0 else int((1 - overlap) * obs_length)
        olap_res = 1 if olap_res < 1 else olap_res
