'''
imitate to rewrite watchped_data.py file.
'''
import sys
import pickle
import cv2
...
class WATCHPED(object):
    ...
    def _get_default_path(self):
        '''
        return default data_path where watchped_raw files are expected to be palced.
        '''
    ...
    def extract_and_save_images(self):
        ...
        # eg: clip_path: watchped_clips/---->video_0001.mp4
        #                           |-->video_0002.mp4
        #                           |-->video_0003.mp4
        #                           |-->video_0004.mp4
        #                           --->video_0005.mp4
        ...
        for vid in videos:
            ...
            # path: watchped_clips/vidXXX.mp4
    ...
    def generate_database(self):
        '''
        Generate a database of watchped database by integrating all annotations
        ...
        cache_file = join(self.cache_path, 'watchped_08.pkl') # watchped_08.pkl是没有acc和gyro的
    ...
    def get_data_stats(self):
        '''
        Generate statistics for watchped database.
        '''
    ...
    def _get_random_pedestrian_ids(self, image_set, ratios=None, val_data=True, regen_data=False, sample_type='all'):
        ...
        if exists(cache_file) and not regen_data:
            ...
            with open(cache_file, 'rb') as fid:
                ...
                if val_data:
                    assert len(rand_samples[
                                   'ratios']) == 3, 'The existing random samples does not have validation data.\n Please try again by setting regen_data = True'
                if ratios is not None:
                    assert ratios == rand_samples['ratios'], 'Specified ratios {} does not match te ones in existing file {}.\n\
                                                                ...
                                                                3- Regenerate data'.format(ratios,
                                                                                           rand_samples['ratios'])
        ...
        with open(cache_file, 'wb') as fid:
            ...
            print('watchped {} samples written to {}'.format('random', cache_file))
    ...
    def _get_kfold_pedestrian_ids(self, image_set, num_folds=5, fold=1, sample_type='all'):
        ...
        if exists(cache_file):
            ...
            with open(cache_file, 'wb') as fid:
                ...
                print('watchped {}-fold samples written to {}'.format(num_folds, cache_file))
    ...
    def get_detection_data(self, image_set, method, occlusion_type=None, file_path='data/', **params):
        ...
        for vid in video_ids:
            ...
            for i in range(0, num_frames, frame_stride):
                ped_samples[join(self._watchped_path, 'image', vid, '{:05d}.png'.format(i))] = []
            for pid in annotations[vid]['ped_annotaions']:
                ...
                if 'p' in pid:
                    ...
                    if image_set in ['train', 'val']:
                        continue
                imgs = [join(self._watchped_path, 'images', vid, '{:05d}.png'.format(f)) for f in
                ...
                for i, b in enumerate(boxes):
                    ...
                    if height_rng[0] <= bbox_height <= height_rng[1]:
                        if (occlusion_type == None and occlusion[i] == 0) or (
                                occlusion_type == 'part' and occlusion[i] < 2) or (occlusion_type == 'full'):
                            if squarify_ratio:
    ...
    def _get_trajectories(self, image_set, annot_database, **params):
        ...
        for vid in sorted(video_ids):
            ...
            for pid in sorted(annot_database[vid]['ped_annotations']):
                ...
                images = [join(self._watchped_path, 'images', vid, '{:05d}.png'.format(f)) for f in
                ...
                if height_rng[0] > 0 or height_rng[1] < float('inf'):
                    images, boxes, frame_ids, occlusions = self._height_check(height_rng, frame_ids, boxes, images,
                                                                              occlusions)
