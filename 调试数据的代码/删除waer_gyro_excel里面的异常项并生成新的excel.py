...
def main():
    ...
    for folder_name in os.listdir(root_dir):
        ...
        gyro_abs_path = os.path.join(folder_path, "WEAR_GYRO_ABSOLUTE_01.xlsx")
        ...
        # =========== 新增：删除异常行并保存到 WEAR_GYRO_ABSOLUTE_02.xlsx ===========
        # df_result 里包含了 is_anomaly, reason 等标记列；我们先去掉异常行
        df_clean = df_result[~df_result["is_anomaly"]].copy()

        # 只保留最初的 8 列（若想保留 dt_prev/dt_next 等可自行调整）
        keep_cols = ["t", "x", "y", "z", "a", "absolute_time", "absolute_time_str", "formatted_time"]
        df_clean = df_clean[keep_cols]

        out_file_02 = os.path.join(folder_path, "WEAR_GYRO_ABSOLUTE_02.xlsx")
        df_clean.to_excel(out_file_02, index=False)
        print(f"[信息] 已生成清洗后文件: {out_file_02} (剩余 {len(df_clean)} 行)")
