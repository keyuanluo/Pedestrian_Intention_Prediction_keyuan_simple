...
def process_acc_file(folder_path, folder_time):
    ...
    try:
        if sep == r'\s+':
            ...
            print(f"[调试] 用空白分隔读取: {acc_file}")
        else:
            df = pd.read_csv(acc_file, sep=sep, header=0)
            print(f"[调试] 用分隔符 '{sep}' 读取: {acc_file}")
    ...
    if 't' not in df.columns:
        print(f"[异常] 缺少 t 列: {acc_file}, 解析到的列名: {df.columns.tolist()}")
        return
    ...
    try:
        ...
        print(f"[异常] 首行 t 无法转换为数值: {acc_file} -> {t0_ns}")
        return
...
def main():
    """
    遍历 /media/robert/4TB-SSD/watchped_dataset/Sensor_acc补充 下的所有子文件夹,
    对以 WEAR_ 开头的文件夹进行同样处理 (process_acc_file).
    """
