...
def parse_image_name(folder_name, is_rainy=False):
    """
    默认情况下 (is_rainy=False):
      文件名形如: "01_06_2022_18_24_49_scene..."
      解析顺序:   日_月_年_时_分_秒

    如果在 "rainy 2" 文件夹下 (is_rainy=True):
      文件名形如: "06_01_2022_18_24_49_scene..."
      解析顺序:   月_日_年_时_分_秒
        ...
        if os.path.isdir(sub_path):
            ...
            for subsub in os.listdir(sub_path):
                ...
                if os.path.isdir(subsub_path):
                    # 检查当前是否是 "rainy 2" 目录
                    # 如果 sub_path 包含 "rainy 2", 我们就认为是 is_rainy=True
                    if "rainy 2" in sub_path:
                        dt_img = parse_image_name(subsub, is_rainy=True)
                    else:
                        dt_img = parse_image_name(subsub, is_rainy=False)
